...
namespace prk2_var7
{
    ...
    *   NOTE: Please, rechange some names in grammar if you want to use it in your report.
    * 
<if statement> ::= IF <logical expr> THEN <state> [ELSE <state>]
<logical expr> ::= <logical realtion expr> {[OR|AND] <logical relation expr> | NOT<logical relation expr>}
<logical relation expr> ::= <expr>{[<|>|<>|=|>=|<=]<expr>}
<expr> ::= <T> {[+|-] <T>}
<T> ::= <F> {[*|/] F}
<F> ::= const | <function> | <id> | (<expr>)
<function> ::=   <id>({<params>})
...
<id> ::= ident{[<F>]} | <id>.<id>
    ...
    */
    ...
    class Program
    {
                    ...
                    if (findIfThen(str)) {
                        ...
                        if (brackets == 0) {
                            if (checkBoolSt(statement)) {
                                string part = "";
                                if (findElse(str)) posStart = elsePos;
                                else posStart = str.Length;
                                for (int i = posEnd + 4; i < posStart; i++) {
                                    if (str[i] != ';') part += str[i];
                                    else {
                                        i++;
                                ...
                                if (checkState(part) && !just) {
                                    ...
                                    if (brackets == 0) {
                                        if (!findElse(str)) {
                                            ...
                                            part = "";
                                            int j = elsePos+4;
                                            for (int i = elsePos + 4; i < str.Length && str[i] != ';'; i++) {
                                                part += str[i];
                                                j++;
                                            }
                                            j++;
                                            ...
                                            if (j>=str.Length) {
                                                part = cleaning(part);
                                                if (part == "") Console.WriteLine("No expression after ELSE.");
                                                else {
                                                    if (checkState(part)) Console.WriteLine("Success");
                                                    else {
                                                        Console.WriteLine("Error in ELSE part.");
                                                        Console.WriteLine(errorMessage);
                                                    }
                                                }
                                            }
                                            else {
                                                Console.WriteLine("There is something after ELSE statement");
                                            }
                                        }
                                    }
                                    else {
        ...
        //find else. Return TRUE if there is else in another case return FALSE
        static bool findElse(string str) {
            string word = "";
            int j = 0;
            for (int i = 0; i < str.Length; i++) {
                if (str[i] != ' ') {
