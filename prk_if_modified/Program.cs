    ...
    static void Main(string[] args) {
                        ...
                        if (checkExpr(statement)) {
                            if (str.ToLower().Contains(" begin ")) {
                                ...
                                if (str.ToLower().Contains(" end")) {
                                    ...
                                    if (cleaning(word) != "") {
                                        if (checkState(word)) Console.WriteLine("No errors. Parse completed successfully.");
                                    }
                                }
                                else {
        ...
        static bool findIfThen(string str) {
            ...
            for (int i = 0; i < str.Length; i++) {
                ...
                if (word.ToLower() == "then" && !thenFounded) {
        ...
        //S[i] in ['A'..'Z']
        static bool checkStructure(string str) {
            if (strongError) return false;
            if (str == "") {
                strongError = true;
                errorMessage += "\nError. missing statement";
            ...
            if (str.IndexOf(" in ") >=0 ) {
                ...
                if (checkF(word)) {
                    word = "";
                    posIn += 4;
                    for (int i = posIn; i<str.Length; i++) {
                        word += str[i];
                    }
                    if (checkArray(word)) {
                        return true;
        ...
        //['A'..'Z']
        static bool checkArray(string str) {
            if (strongError) return false;
            if (str == "") {
                strongError = true;
                errorMessage += "\nError. missing statement";
            ...
            if (str[0] == '[' && str[str.Length-1] == ']' ) {
                for (int i=1; i<str.Length-1; i++) {
                    if (str[i].ToString() == "'") isConst = !isConst;
                    if (str[i] == '.' && !isConst) {
                        dotCount++;
                        if (str[i + 1] != '.' && dotCount < 2) {
                            return false;
                        }
                        else {
                            if (str[i + 1] == '.' && dotCount == 2) return false;
                            else if (str[i + 1] == '.') {
                                dotCount++;
                                i++;
                            }
                ...
                if (isConst) {
                    strongError = true;
                    errorMessage += "\nError. String const have no end.";
                    return false;
                }
            }
            else {
